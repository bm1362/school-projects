...
$(document).ready(function () {
	...
	function whileLoading() {
		if(document.loading) {
			var txt = $("#loading-status").text();
			var ellips = txt.lastIndexOf('.')
			var ellips = txt.length - 7 + 1;
			...
			var txt = 'Loading';
			for( var i = 0; i < ellips; i++)
				txt += '.';

			$("#loading-status").text(txt);
			setTimeout(whileLoading, 250);
		}
		else
			$("#loading").remove();
	}
